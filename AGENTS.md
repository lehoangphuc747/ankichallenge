...
- **Ngoại lệ auto-approve**: Nếu người đăng ký AC11 đã từng tham gia **AC10** (`challengeIds` chứa `3`) → bot **tự động** duyệt + gán role ngay khi đăng ký (không cần admin duyệt). Chỉ người **chưa từng AC10** mới phải qua bước duyệt thủ công ở `/admin/registrations`. (Xử lý trong `register.ts`.)
