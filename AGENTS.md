...
- `/checkin` xác thực `discordId` ↔ `users` KV/D1, kiểm tra **Thread Gate** (nếu gọi trong thread ngày `D{x}-{DD}/{MM}`, bắt buộc ngày check-in phải khớp ngày thread để tránh gõ nhầm ngày; nếu lệch sẽ từ chối và hướng dẫn sang đúng thread), sau đó ghi nhận record. **Option `image` là BẮT BUỘC (2026-09-03)**: phải kèm ảnh (`ATTACHMENT`, `required:true`) mới check-in được; lưu `imageUrl = attachment.url` (link CDN vĩnh viễn của Discord) vào D1 `checkins`, **không re-upload** lên CDN khác. Option `minutes` là kiểu `NUMBER` (type 10) hỗ trợ số thực thập phân. Tái đăng ký command qua `scripts/deploy-commands.js` sau khi đổi.
