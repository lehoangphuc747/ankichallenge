...
const CHALLENGE_FILE_KEY = '10';

function parseArgs(argv) {
  const args = {
    all: false,
    userId: null,
    outDir: 'output/certificates-ch10',
    format: 'png',
    merge: false,
  };
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--out') {
      const value = argv[i + 1];
      if (!value) {
        throw new Error('Gia tri --out khong hop le. Vi du: --out output/certificates-ch10');
      }
      args.outDir = value;
      i += 1;
      continue;
...
async function exportSingleImage(browser, payload, targetPath, format) {
...
async function main() {
  ...
  const ext = args.format === 'pdf' ? 'pdf' : (args.format === 'jpg' ? 'jpg' : 'png');
  const formatLabel = { png: 'PNG', jpg: 'JPG', pdf: 'PDF' }[args.format];

  console.log(`Challenge: ${challenge.name} (${CHALLENGE_ID})`);
  console.log(`So thanh vien duoc xuat: ${selectedUsers.length}`);
  console.log(`Dinh dang: ${formatLabel}`);
  ...
  try {
    for (const user of selectedUsers) {
      ...
      if (args.format === 'pdf') {
        ...
        await exportSingleImage(browser, {
          userName: user.name,
          challengeName: challenge.name,
          studyDays: stats.studyDays,
          totalDays: stats.totalDays,
